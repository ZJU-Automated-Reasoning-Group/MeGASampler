...
public:
  ...
  void run() {
    ...
    while (true) { // bat: each iteration is an epoch. Will exit inside solve()
                   // or sample().
      opt.push();  // because formula is constant, but other hard/soft
                   // constraints change between epochs
      solver.push();
      for (z3::func_decl &v :
           ind) { // bat: Choose a random assignment: for variable-> if bv or
  ...
  void assert_soft(z3::expr const &e) { opt.add(e, 1); }
  ...
  /*
   * Find all variables in formula. Count variables of different sorts.
  ...
  void parse_smt() {
    ...
    if (convert) {
      ...
      opt.add(formula);    // adds formula as hard constraint to optimization
                           // solver (no weight specified for it)
      solver.add(formula); // adds formula as constraint to normal solver
      z3::check_result result = solve(); // will try to solve the formula and
      ...
      evaluate(model, smt_formula, true,
               1); // will evaluate smt_formula under model, with
                   // model_completion=true and coverage_enable=1
      // result from evaluate is not checked since in this case the model must
      // satisfy the formula (?)
    }
  ...
  void sample(z3::model m) {
    ...
    for (int count = 0; count < constraints.size(); ++count) {
      ...
      z3::check_result result = z3::unknown;
      if (cost * rand() <=
          (max_time / 3.0 + start_epoch - elapsed) * RAND_MAX) {
        result = solve();
        ++calls;
      }
      if (result == z3::sat) {
        ...
        for (std::string c_string : initial) {
          ...
          for (z3::func_decl &w : ind) {
            if (w.range().is_array()) {
              ...
              combine_function(m_string, b_string, c_string, pos_a, pos_b,
                               pos_c, arity, s, candidate);
            } else if (w.is_const()) {
              z3::sort s = w.range();
              std::string num =
  ...
  bool is_ind(int count) { return !flip_internal || count >= internal.size(); }
  ...
  z3::model gen_model(std::string candidate, std::vector<z3::func_decl> ind) {
    ...
    for (z3::func_decl &v : ind) {
      if (v.range().is_array()) {
        ...
        for (int j = 0; j < num; ++j) {
          z3::expr arg =
              value(candidate.c_str() + pos, v.range().array_domain());
          pos = candidate.find('\0', pos) + 1;
          z3::expr val =
              value(candidate.c_str() + pos, v.range().array_range());
          pos = candidate.find('\0', pos) + 1;
   ...
   * if valid -> insert to all_mutations set (collects unique valid samples),
   * valid samples ++, if valid and new -> print to results file
   */
  bool output(std::string sample, int nmut) {
    ...
    z3::expr b = evaluate(m, smt_formula, true,
                          0); // evaluates smt_formula under m with
                              // model_completion=true and coverage_enable=0
    ...
    if (valid) {
      ...
      evaluate(m, smt_formula, true,
               2); // only if m is a valid solution, calculate coverage (with 2,
                   // not 1 - why?)
    } else if (nmut <= 1) {
  ...
  z3::check_result solve() {
    ...
    if (result == z3::sat) {
      ...
      std::cout << "MAX-SMT timed out"
                << "\n";
      try {
        result = solver.check(); // bat: if too long, solve a regular SMT
                                 // instance (without any soft constraints)
      } catch (z3::exception except) {
        std::cout << "Exception: " << except << "\n";
        exit(1);
      }
